<x-layouts.app title="Notifikasi">
    <div class="min-h-screen bg-white rounded-xl shadow-md p-6" x-data="notificationApp(@js($globalNotifications ?? []))">
        ...
        <div class="flex flex-col md:flex-row gap-3 mb-6">
            <input type="text" x-model="search" placeholder="Cari notifikasi..."
                class="hidden w-full md:w-1/2 px-4 py-2 border rounded-lg focus:ring-indigo-500 focus:border-indigo-500 text-sm">

            <div class="flex gap-2">
                <button @click="filter = 'all'" :class="filter === 'all' ? 'bg-indigo-600 text-white' : 'bg-white'"
        ...
        <div class="space-y-3">
            ...
            <template x-for="notif in filteredNotifications()" :key="notif.id">
                <div class="bg-white rounded-xl shadow-sm p-4 flex items-start justify-between 
                    ...
                    :class="!notif.pivot.is_read ? 'bg-indigo-50/60' : ''">
                    ...
                    <div class="flex gap-3">
                        ...
                        <!-- CONTENT -->
                        <div>
                            <p class="text-sm font-semibold text-gray-800" x-text="notif.title"></p>
                            ...
                            <div class="flex items-center gap-2 mt-2">
                                ...
                                <span x-show="!notif.pivot.is_read" class="w-2 h-2 bg-indigo-500 rounded-full"></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <!-- MARK READ -->
                        <button @click="markAsRead(notif.id)" class="p-2 rounded hover:bg-green-100 text-green-600"
                            title="Tandai Dibaca">
                            <i class="ri-check-line"></i>
                        </button>

                        <!-- DELETE -->
                        <button @click="remove(notif.id)" class="hidden p-2 rounded hover:bg-red-100 text-red-600"
                            title="Hapus">
            ...
            <!-- EMPTY -->
            <div x-show="filteredNotifications().length === 0" class="text-center text-gray-500 py-10">
    ...
    <script>
        function notificationApp(initialData) {
            ...
            return {
                ...
                filteredNotifications() {
                    return this.notifications.filter(n => {
                        ...
                        if (this.filter === 'unread') {
                            return !n.pivot.is_read && matchSearch;
                        }
                ...
                unreadCount() {
                    return this.notifications.filter(n => !n.pivot.is_read).length;
                },
